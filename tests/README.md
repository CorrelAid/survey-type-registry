...
`examples/<id>/ddi.xml` and `examples/<id>/tsv.tsv` are **frozen reference snapshots**, not auto-regenerated by `codegen.py`. `test_snapshots.py` runs the current driver and asserts its output matches the committed snapshot. A driver version bump that changes output → snapshot test failure.

When that happens, decide:

- **Driver change is correct** → bless new output:
  ```bash
  bash scripts/bless-snapshots.sh   # rewrites ddi.xml + tsv.tsv via current drivers
  git diff examples/                 # inspect
  git add examples/ && git commit -m "bless: regen snapshots for survey2ddi vX.Y"
  ```
- **Driver change is a regression** → fix the driver, leave snapshots alone.

Default `python codegen.py` only writes `meta.json` and `xlsform.xlsx` (registry-derived, deterministic from JSON-LD). It does NOT touch `ddi.xml` / `tsv.tsv`. Drift guard reflects this: changes to those files won't be auto-corrected.

## Known limitations

1. **Single-driver snapshots are still semi-tautological at any one SHA.** `codegen.py --bless-snapshots` uses the same `survey2ddi_core.ddi_xml.build_ddi_xml` that `test_snapshots.py` calls. Within one CI run at one driver version, the assertion is trivially true. Non-tautology comes from *time*: when a future driver version produces different output against the same committed snapshot, the test fails until someone re-blesses. The bless step IS the human gate.
