...
def generate_type_docs(registry: dict[str, Any], base_dir: Path) -> int:
    """For each v1-blessed QuestionType (has tier='v1-blessed'), write
    types/<slug>/docs.md with workflow narrative + lifecycle mermaid.
    ...
    variants_by_id = {
        e["@id"]: e for e in registry.values() if e.get("@type") == "PresentationVariant"
    }

    written = 0
    for entry in blessed:
        slug = entry["xlsform"]["typeString"]
        tdir = base_dir / "types" / slug
        if not tdir.exists():
            continue
        ...
        round_trip = entry.get("transformation", {}).get("roundTripSafe", False)
        lossless = entry.get("transformation", {}).get("lossless", False)
        c = entry.get("concept", {})
        constraints = entry.get("constraints", {})
        warnings = entry.get("transformation", {}).get("warnings", [])
        ...
        for vid in entry.get("variants", []):
            ...
            if pres.get("appearance"):
                flags.append(f"appearance={pres['appearance']}")
            flag_str = ", ".join(flags) or "—"
            variant_rows.append(
                f"| [`{vslug}`](examples/{vslug}/) | {vlabel} | {flag_str} |"
            )
        ...
        warning_lines = "\n".join(f"- ⚠️ {w}" for w in warnings) or "_None._"
...
```"""
        ...
        doc = f"""<!-- GENERATED by codegen.py — DO NOT EDIT BY HAND.
     Edit `types/{slug}/definition.jsonld` and re-run `python codegen.py`. -->

# {label} (`{slug}`)

**Tier:** {entry.get('tier', '—')} · **Frozen since:** {entry.get('frozenSince', '—')}

## Concept

- openness: `{c.get('openness', '—')}`
- cardinality: `{c.get('cardinality', '—')}`
- dataNature: `{c.get('dataNature', '—')}`
...
| qwacback `answerType` | `{entry.get('qwacback', {}).get('answerType', '—')}` |
...
| Property | Value |
|----------|-------|
| roundTripSafe | {'✅' if round_trip else '⚠️'} |
| lossless | {'✅' if lossless else '⚠️'} |
...
if __name__ == "__main__":
    ...
    print(f"  ✅ types/<slug>/docs.md ({n_docs} blessed types)")
